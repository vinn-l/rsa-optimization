...
// Optimization: Attempt Neon Operation for add_uint32x3
// Resulted in a significant increase in cycle count and run time
// uint32x3_t add_uint32x3(uint32x3_t x, uint32x3_t y)
// {
//     uint32x4_t result;

//     // Memory accesses here cost a lot, attempted to avoid this, but NEON load intrinsics seems to only supports loading from memory.
//     uint32_t x_4[4] = {x.low, x.mid, x.high, 0};
//     uint32_t y_4[4] = {y.low, y.mid, y.high, 0};
//     uint32x4_t x_neon = vld1q_u32(x_4);
//     uint32x4_t y_neon = vld1q_u32(y_4);

//     result = vaddq_u32(x_neon, y_neon);

//     uint32_t high = vgetq_lane_u32(result, 2);
//     uint32_t mid = vgetq_lane_u32(result, 1);
//     uint32_t low = vgetq_lane_u32(result, 0);
...
//     uint32x3_t return_result = {high, mid, low};
...
uint32x3_t sub_uint32x3(uint32x3_t x, uint32x3_t y)
{
    ...
    // Optimization: Loop Unrolling
    tmp1 = (uint64_t)x.low + ((uint64_t)0xFFFFFFFF + 1); /* + number_base */
    tmp2 = (uint64_t)y.low + borrow;
    res = (tmp1 - tmp2);
    result.low = (uint32_t)(res & (uint64_t)0xFFFFFFFF); /* "modulo number_base" == "% (number_base - 1)" if number_base is 2^N */
    borrow = (res <= (uint64_t)0xFFFFFFFF);

    tmp1 = (uint64_t)x.mid + ((uint64_t)0xFFFFFFFF + 1); /* + number_base */
    tmp2 = (uint64_t)y.mid + borrow;
    res = (tmp1 - tmp2);
    result.mid = (uint32_t)(res & (uint64_t)0xFFFFFFFF); /* "modulo number_base" == "% (number_base - 1)" if number_base is 2^N */
    borrow = (res <= (uint64_t)0xFFFFFFFF);

    tmp1 = (uint64_t)x.high + ((uint64_t)0xFFFFFFFF + 1); /* + number_base */
    tmp2 = (uint64_t)y.high + borrow;
    res = (tmp1 - tmp2);
    result.high = (uint32_t)(res & (uint64_t)0xFFFFFFFF); /* "modulo number_base" == "% (number_base - 1)" if number_base is 2^N */
    borrow = (res <= (uint64_t)0xFFFFFFFF);
...
uint32x3_t modular_multiplication_32x3(uint32x3_t X, uint32x3_t Y, uint32x3_t M, uint32_t numBits)
{
    ...
    register uint32x3_t T_low = {0};
    register uint32_t T_high = 0;
    register int m = numBits;
    register uint32_t n;

    // The lowest bit of X_local is i-th bit
    register uint32_t X_local;
    register uint32_t Y_0 = Y.low & 1;

    // Optimization: The algorithm only requires the i-th bit of X and Y, therefore a 32-bit register can be used to store this.
    for (int i = 0; i < m; i++)
    {
        // Only grab the relevant 32-bits that is required (low 32-bits, mid 32-bits or high 32-bits)

        // If i == 0, then get low 32-bits for X_local
        // If i == 32, then get mid 32-bits for X_local
        // If i == 64, then get high 32-bits for X_local
        if(!(i % 32)){
            // This occurs only once every 31 loop iterations
            X_local = X.low;
            X = rshift_uint32x3(X, 32);
        }

        // n = T(0) XOR (X(i) AND Y(0))
        n = (T_low.low & 1) ^ ((X_local & 1) & Y_0);
        ...
        // Shift X_local down by 1 for every iteration to ensure i-th bit is always the lowest bit
        X_local >>= 1;
