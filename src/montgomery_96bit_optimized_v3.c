...
uint32x3_t rshift1_uint32x3(uint32x3_t);
uint32x3_t rshift32_uint32x3(uint32x3_t);
...
// Optimization: Since we know that rshift_uint32x3 is only used with constant 1 and constant 32
...
uint32x3_t rshift1_uint32x3(uint32x3_t x)
{
    uint32x3_t result = {0};
...
uint32x3_t rshift32_uint32x3(uint32x3_t x)
{
    uint32x3_t result = {0};
    ...
    // Compared to the previous iteration of rshift_uint32x3, we save a few cycles and code memory.
    result.low = x.mid;
    result.mid = x.high;
    result.high = 0;
    return result;
...
uint32x3_t modular_exponentiation_mont_32x3(uint32x3_t b, uint32x3_t e, uint32x3_t m, int numBits, uint32x3_t r2_mod)
{
    uint32x3_t ONE = {0,0,1};
    uint32x3_t result = modular_multiplication_32x3(ONE, r2_mod, m, numBits);
    uint32x3_t p = modular_multiplication_32x3(b, r2_mod, m, numBits);
...
uint32x3_t modular_multiplication_32x3(uint32x3_t X, uint32x3_t Y, uint32x3_t M, int numBits)
{
    ...
    // therefore a 32-bit can be used to store relevent 32-bit chunks (high, mid, or low) of 96-bit X.
    uint32_t X_32local;

    // Optimization: The index required from Y is always the 0-th index, or Y(0), thus this can be saved into a and be reused for all loop iterations.
    ...
    for (int i = 0; i < numBits; i++)
    {
        ...
        // and will potentially save us registers because X_32local only requires 1 whereas X requires 3.
        if(!(i % 32)){
            // This occurs only once every 32 loop iterations
            X_32local = X.low;
            X = rshift32_uint32x3(X);
        }
...
int main()
{
    ...
    uint32x3_t r2_mod_48_2 = {0, 0x0000265b, 0xa323e7d0};
    ...
    t = clock();
    uint32x3_t cipher_t = encrypt(plain_t, E, N, 48, r2_mod_48_2);
    t = clock() - t;
    total += t;
    ...
    t = clock();
    uint32x3_t plain_t_res = decrypt(cipher_t, D, N, 48, r2_mod_48_2);
    t = clock() - t;
    total += t;
    time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
    printf("Time taken = %fs, Clock Cycles = %ld\n", time_taken, t);
    ...
    uint32x3_t r2_mod_96_2 = {0x599147d0, 0x0a1404f1 ,0xb605d62c};
    ...
    t = clock();
    uint32x3_t cipher_t_96_2 = encrypt(plain_t_96_2, E_96_2, N_96_2, 96, r2_mod_96_2);
    t = clock() - t;
    total += t;
    time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
    printf("Time taken = %fs, Clock Cycles = %ld\n", time_taken, t);
    ...
    t = clock();
    uint32x3_t plain_t_res_96_2 = decrypt(cipher_t_96_2, D_96_2, N_96_2, 96, r2_mod_96_2);
    t = clock() - t;
    total += t;
    time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
    printf("Time taken = %fs, Clock Cycles = %ld\n", time_taken, t);
